...
void ReportError(LPTSTR szMsg) {
	...
	TRACE(szErrorFormat, GetLastError(), szMsg);
}
...
HANDLE OpenVolume(TCHAR cDriveLetter) {
	...
	default:
		...
		TRACE(TEXT("Cannot eject.  Drive type is incorrect.\n"));
...
BOOL EjectVolume(TCHAR cDriveLetter) {
	...
	if(fAutoEject) {
		TRACE("Media in Drive %c has been ejected safely.\n",
			cDriveLetter);
		return TRUE;
	}
	else if(fRemoveSafely) {
		TRACE("Media in Drive %c can be safely removed.\n",
			cDriveLetter);
		return TRUE;
	}
